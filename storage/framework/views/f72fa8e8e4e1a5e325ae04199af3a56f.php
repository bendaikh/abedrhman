...
unset($__errorArgs, $__bag); ?>
                ...
                <!-- Client Info Block -->
                <div id="client-info-block" class="hidden bg-gradient-to-br from-slate-50 to-teal-50 dark:from-gray-700/50 dark:to-teal-900/20 rounded-xl p-5 border border-teal-200 dark:border-teal-800">
                    <div class="space-y-5">
                        <!-- CLIENT Section -->
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Nom / Raison sociale</p>
                                    <p id="client-nom" class="text-sm font-semibold text-gray-900 dark:text-white">-</p>
                                </div>
                                <div>
                                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Forme juridique</p>
                                    <p id="client-forme-juridique" class="text-sm font-semibold text-gray-900 dark:text-white">-</p>
                                </div>
                                <div>
                                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Activités</p>
                                    <p id="client-activite" class="text-sm font-semibold text-gray-900 dark:text-white">-</p>
                                </div>
                                <div>
                                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">N° pièce</p>
                                    <p id="client-numero-piece" class="text-sm font-semibold text-gray-900 dark:text-white">-</p>
                                </div>
                                <div class="md:col-span-2">
                                    <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Siège</p>
                                    <p id="client-siege" class="text-sm font-semibold text-gray-900 dark:text-white">-</p>
                                </div>
                            </div>
                        </div>

                        <!-- DIRIGEANTS Section -->
                        <div id="dirigeants-section" class="hidden">
                            <h4 class="text-sm font-bold text-gray-900 dark:text-white mb-3 flex items-center gap-2">
                                <svg class="w-4 h-4 text-teal-600 dark:text-teal-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div id="dirigeants-list" class="space-y-3">
                                <!-- Dirigeants will be populated here by JavaScript -->
                            </div>
                        </div>
                    </div>
                </div>
...
function showClientInfo(clientId) {
    ...
    const dirigeantsSection = document.getElementById('dirigeants-section');
    const dirigeantsList = document.getElementById('dirigeants-list');
    const selectedOption = select.options[select.selectedIndex];
    
    if (clientId && selectedOption) {
        const nom = selectedOption.dataset.nom || '-';
        const formeJuridique = selectedOption.dataset.formeJuridique || 'N/A';
        const activite = selectedOption.dataset.activite || 'N/A';
        const numeroPiece = selectedOption.dataset.numeroPiece || 'N/A';
        const siegeSocial = selectedOption.dataset.siegeSocial || 'N/A';
        const type = selectedOption.dataset.type;
        
        // Update CLIENT information
        document.getElementById('client-nom').textContent = nom;
        document.getElementById('client-forme-juridique').textContent = formeJuridique;
        document.getElementById('client-activite').textContent = activite;
        document.getElementById('client-numero-piece').textContent = numeroPiece;
        document.getElementById('client-siege').textContent = siegeSocial;
        
        // Handle DIRIGEANTS
        let dirigeants = [];
        try {
            const dirigeantsData = selectedOption.dataset.dirigeants;
            if (dirigeantsData) {
                dirigeants = JSON.parse(dirigeantsData);
            }
        } catch (e) {
            console.error('Error parsing dirigeants data:', e);
        }
        
        // Clear existing dirigeants
        dirigeantsList.innerHTML = '';
        
        if (dirigeants && dirigeants.length > 0 && type !== 'particulier') {
            dirigeants.forEach((dirigeant, index) => {
                const dirigeantCard = document.createElement('div');
                dirigeantCard.className = 'bg-white dark:bg-gray-800 rounded-lg p-3 border border-gray-200 dark:border-gray-600';
                
                const grid = document.createElement('div');
                grid.className = 'grid grid-cols-1 md:grid-cols-4 gap-3';
                
                // Helper function to create a field
                const createField = (label, value) => {
                    const fieldDiv = document.createElement('div');
                    const labelP = document.createElement('p');
                    labelP.className = 'text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1';
                    labelP.textContent = label;
                    const valueP = document.createElement('p');
                    valueP.className = 'text-sm font-semibold text-gray-900 dark:text-white';
                    valueP.textContent = value || '-';
                    fieldDiv.appendChild(labelP);
                    fieldDiv.appendChild(valueP);
                    return fieldDiv;
                };
                
                grid.appendChild(createField('Nom', dirigeant.nom));
                grid.appendChild(createField('Prénom', dirigeant.prenom));
                grid.appendChild(createField('Fonction', dirigeant.fonction));
                grid.appendChild(createField('Tel', dirigeant.tel));
                
                dirigeantCard.appendChild(grid);
                dirigeantsList.appendChild(dirigeantCard);
            });
            dirigeantsSection.classList.remove('hidden');
        } else {
            dirigeantsSection.classList.add('hidden');
        }
