    ...
    <form action="{{ isset($actionnaire) ? route('actionnaires.update', [$client->id, $actionnaire->id]) : route('actionnaires.store', $client->id) }}" method="POST" class="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow border border-gray-100 dark:border-gray-700 p-4 sm:p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div>
                ...
                <select name="type_piece_id" id="type_piece_id_select" onchange="toggleNPieceIdField()" class="w-full px-3 sm:px-4 py-2 text-sm sm:text-base border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                    <option value="">Sélectionner</option>
                    <option value="CIN" {{ old('type_piece_id', $actionnaire->type_piece_id ?? '') === 'CIN' ? 'selected' : '' }}>CIN</option>
                    <option value="PASSPORT" {{ old('type_piece_id', $actionnaire->type_piece_id ?? '') === 'PASSPORT' ? 'selected' : '' }}>PASSPORT</option>
                    <option value="CARTE SEJOUR" {{ old('type_piece_id', $actionnaire->type_piece_id ?? '') === 'CARTE SEJOUR' ? 'selected' : '' }}>CARTE SEJOUR</option>
                    <option value="CARTE ETRANGERE" {{ old('type_piece_id', $actionnaire->type_piece_id ?? '') === 'CARTE ETRANGERE' ? 'selected' : '' }}>CARTE ETRANGERE</option>
                </select>
            </div>

            <!-- N° pièce ID -->
            <div id="n_piece_id_field" style="display: {{ old('type_piece_id', $actionnaire->type_piece_id ?? '') ? 'block' : 'none' }};">
                <label class="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">N° pièce ID</label>
                <input type="text" name="n_piece_id" value="{{ old('n_piece_id', $actionnaire->n_piece_id ?? '') }}" 
                    class="w-full px-3 sm:px-4 py-2 text-sm sm:text-base border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent">
            </div>
