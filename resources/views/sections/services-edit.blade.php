        ...
        <form action="{{ route('services.update', $service) }}" method="POST" class="space-y-6">
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <!-- Date du service -->
                    <div>
                        <label for="date_service" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Date du service</label>
                        <input type="date" name="date_service" id="date_service" 
                            value="{{ old('date_service', $service->date_service ? $service->date_service->format('Y-m-d') : '') }}"
                            class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-colors">
                        @error('date_service')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
