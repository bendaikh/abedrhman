...
@section('title', 'Factures - Abedrhman')
...
<div class="space-y-6 sm:space-y-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-100 dark:border-gray-700 p-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-lg font-bold text-gray-900 dark:text-white">{{ $totalFactures }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-100 dark:border-gray-700 p-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-lg font-bold text-gray-900 dark:text-white">{{ number_format($totalMontant, 2, ',', ' ') }} DH</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-100 dark:border-gray-700 p-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-lg font-bold text-emerald-600 dark:text-emerald-400">{{ number_format($totalPaye, 2, ',', ' ') }} DH</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-100 dark:border-gray-700 p-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-lg font-bold text-gray-900 dark:text-white">{{ $facturesPayees }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- Filters -->
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-100 dark:border-gray-700 p-4">
        <form method="GET" action="{{ route('factures.index') }}" class="flex flex-wrap gap-4">
            <div class="flex-1 min-w-[200px]">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Rechercher par numéro ou client..."
            ...
            <div>
                <select name="statut" class="px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-violet-500 focus:border-violet-500">
                    ...
                    <option value="partielle" {{ request('statut') == 'partielle' ? 'selected' : '' }}>Partiellement payée</option>
            ...
            <a href="{{ route('factures.index') }}" class="px-4 py-2 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 font-medium rounded-lg transition-colors">
                Réinitialiser
            </a>
            @endif
        </form>
        ...
        <div class="p-4 sm:p-6 border-b border-gray-100 dark:border-gray-700 bg-violet-50 dark:bg-violet-900/20">
            <div class="flex items-center gap-2">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ $factures->total() }} facture(s)</p>
                </div>
            </div>
        </div>
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        <td class="px-4 py-4 whitespace-nowrap">
                            <span class="text-sm font-semibold text-violet-600 dark:text-violet-400">{{ $facture->numero }}</span>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap">
                            @if($facture->client)
                            <div class="flex items-center gap-3">
                                <div class="h-8 w-8 rounded-full bg-violet-100 dark:bg-violet-900/30 flex items-center justify-center flex-shrink-0">
                                    <span class="text-sm font-medium text-violet-600 dark:text-violet-400">
                                        {{ strtoupper(substr($facture->client->type === 'morale' ? $facture->client->nom_raison_sociale : $facture->client->nom, 0, 1)) }}
                                    </span>
                                </div>
                                <div>
                        ...
                        <td class="px-4 py-4 whitespace-nowrap">
                            <span class="text-sm text-gray-600 dark:text-gray-300">{{ $facture->service->typeService->nom ?? 'N/A' }}</span>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap">
                            <span class="text-sm text-gray-600 dark:text-gray-300">{{ $facture->date_facture->format('d/m/Y') }}</span>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap">
                            <div class="flex flex-col">
                                ...
                                <span class="text-xs text-amber-600 dark:text-amber-400">Reste: {{ $facture->formatted_montant_restant }}</span>
                                @endif
                            </div>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap text-center">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $facture->statut_color }}">
                                {{ $facture->statut_label }}
                            </span>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end gap-1">
                                <a href="{{ route('factures.show', $facture) }}" class="p-2 text-violet-600 hover:text-violet-700 dark:text-violet-400 dark:hover:text-violet-300 hover:bg-violet-50 dark:hover:bg-violet-900/20 rounded-lg transition-colors" title="Voir">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                    </svg>
                                </a>
                                <a href="{{ route('factures.print', $facture) }}" target="_blank" class="p-2 text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors" title="Imprimer">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
                                    </svg>
                                </a>
                                <form action="{{ route('factures.destroy', $facture) }}" method="POST" class="inline" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette facture ?');">
                                    @csrf
                                    @method('DELETE')
