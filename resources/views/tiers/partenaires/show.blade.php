...
@section('title', 'Détails du partenaire - Abedrhman')
...
<div class="space-y-6">
    <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
        <div class="flex items-center gap-3">
            ...
            <h2 class="text-xl sm:text-2xl font-semibold text-gray-900 dark:text-white">Détails du partenaire</h2>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('partenaires.edit', $partenaire->id) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors">
                Modifier
            </a>
            <form action="{{ route('partenaires.destroy', $partenaire->id) }}" method="POST" class="inline" onsubmit="return confirm('Êtes-vous sûr ?');">
                @csrf
                @method('DELETE')
        ...
        <div class="p-6">
            <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <dd class="text-base text-gray-900 dark:text-white">{{ $partenaire->raison_sociale ?? 'N/A' }}</dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Activité</dt>
                    <dd class="text-base text-gray-900 dark:text-white">{{ $partenaire->activite ?? 'N/A' }}</dd>
                </div>
            </dl>
            ...
            <div class="mt-6 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-200 dark:border-gray-600">
                ...
                <dl class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <dd class="text-base text-gray-900 dark:text-white">{{ $partenaire->responsable_nom ?? 'N/A' }}</dd>
                    </div>
                    <div>
                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Prénom</dt>
                        <dd class="text-base text-gray-900 dark:text-white">{{ $partenaire->responsable_prenom ?? 'N/A' }}</dd>
                    </div>
                    <div>
                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Fonction</dt>
                        <dd class="text-base text-gray-900 dark:text-white">{{ $partenaire->fonction ?? 'N/A' }}</dd>
                    </div>
                </dl>
            </div>
            ...
            <dl class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                    ...
                    <dd class="text-base text-gray-900 dark:text-white">{{ $partenaire->tel ?? 'N/A' }}</dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Email</dt>
                    <dd class="text-base text-gray-900 dark:text-white">{{ $partenaire->email ?? 'N/A' }}</dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">ICE</dt>
                    <dd class="text-base text-gray-900 dark:text-white">{{ $partenaire->ice ?? 'N/A' }}</dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">RIB</dt>
                    <dd class="text-base text-gray-900 dark:text-white">{{ $partenaire->rib ?? 'N/A' }}</dd>
                </div>
                <div class="md:col-span-2">
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Adresse</dt>
                    <dd class="text-base text-gray-900 dark:text-white">{{ $partenaire->adresse ?? 'N/A' }}</dd>
                </div>
            </dl>
        </div>
    </div>
