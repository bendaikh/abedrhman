...
@section('title', ($comptable ? 'Modifier' : 'Créer') . ' un comptable - Abedrhman')
...
<div class="space-y-6">
    <div class="flex items-center gap-3">
        ...
        <h2 class="text-xl sm:text-2xl font-semibold text-gray-900 dark:text-white">
            {{ $comptable ? 'Modifier' : 'Créer' }} un comptable
        </h2>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow border border-gray-100 dark:border-gray-700 p-6">
        <form action="{{ $comptable ? route('comptables.update', $comptable->id) : route('comptables.store') }}" method="POST">
            @csrf
            @if($comptable)
                @method('PUT')
            @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="raison_sociale" name="raison_sociale" value="{{ old('raison_sociale', $comptable->raison_sociale ?? '') }}"
                ...
                <div>
                    <label for="prestation" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Prestation</label>
                    <input type="text" id="prestation" name="prestation" value="{{ old('prestation', $comptable->prestation ?? '') }}"
                        class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                ...
                <div>
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email', $comptable->email ?? '') }}"
                        class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500">
                </div>

                <div>
                    <label for="ice" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">ICE</label>
                    <input type="text" id="ice" name="ice" value="{{ old('ice', $comptable->ice ?? '') }}"
                        class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500">
                </div>

                <div>
                    <label for="rib" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">RIB</label>
                    <input type="text" id="rib" name="rib" value="{{ old('rib', $comptable->rib ?? '') }}"
                        class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="adresse" name="adresse" rows="3"
                        class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500">{{ old('adresse', $comptable->adresse ?? '') }}</textarea>
                </div>
            </div>

            <div class="flex items-center justify-end gap-3 mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                <a href="{{ route('comptables.index') }}" class="px-6 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                    Annuler
                </a>
                <button type="submit" class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors">
                    {{ $comptable ? 'Mettre à jour' : 'Créer' }}
                </button>
            </div>
        </form>
    </div>
