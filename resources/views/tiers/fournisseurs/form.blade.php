...
@section('title', ($fournisseur ? 'Modifier' : 'Créer') . ' un fournisseur - Abedrhman')
...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <h2 class="text-xl sm:text-2xl font-semibold text-gray-900 dark:text-white">
            {{ $fournisseur ? 'Modifier' : 'Créer' }} un fournisseur
        </h2>
    </div>

    <!-- Form -->
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow border border-gray-100 dark:border-gray-700 p-6">
        <form action="{{ $fournisseur ? route('fournisseurs.update', $fournisseur->id) : route('fournisseurs.store') }}" method="POST">
            @csrf
            @if($fournisseur)
                @method('PUT')
            @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="activite" name="activite" 
                        value="{{ old('activite', $fournisseur->activite ?? '') }}"
                        class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    @error('activite')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="responsable_nom" name="responsable_nom" 
                            value="{{ old('responsable_nom', $fournisseur->responsable_nom ?? '') }}"
                            class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    <div>
                        ...
                        <input type="text" id="responsable_prenom" name="responsable_prenom" 
                            value="{{ old('responsable_prenom', $fournisseur->responsable_prenom ?? '') }}"
                            class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    <div>
                        ...
                        <input type="text" id="fonction" name="fonction" 
                            value="{{ old('fonction', $fournisseur->fonction ?? '') }}"
                            class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        @error('fonction')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="adresse" name="adresse" rows="3"
                        class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('adresse', $fournisseur->adresse ?? '') }}</textarea>
                    @error('adresse')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
