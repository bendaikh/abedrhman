...
@section('title', 'Détails du compte associé - Abedrhman')
...
<div class="space-y-6">
    <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
        <div class="flex items-center gap-3">
            ...
            <h2 class="text-xl sm:text-2xl font-semibold text-gray-900 dark:text-white">Détails du compte associé</h2>
        </div>
        <div class="flex gap-2">
            <a href="{{ route('comptes-associes.edit', $compteAssocie->id) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors">
                Modifier
            </a>
            <form action="{{ route('comptes-associes.destroy', $compteAssocie->id) }}" method="POST" class="inline" onsubmit="return confirm('Êtes-vous sûr ?');">
                @csrf
                @method('DELETE')
        ...
        <div class="p-6">
            <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <dd class="text-base text-gray-900 dark:text-white">{{ $compteAssocie->tel ?? 'N/A' }}</dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Email</dt>
                    <dd class="text-base text-gray-900 dark:text-white">{{ $compteAssocie->email ?? 'N/A' }}</dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">RIB</dt>
                    <dd class="text-base text-gray-900 dark:text-white">{{ $compteAssocie->rib ?? 'N/A' }}</dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Date de création</dt>
                    <dd class="text-base text-gray-900 dark:text-white">{{ $compteAssocie->created_at->format('d/m/Y H:i') }}</dd>
                </div>
                <div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Dernière modification</dt>
                    <dd class="text-base text-gray-900 dark:text-white">{{ $compteAssocie->updated_at->format('d/m/Y H:i') }}</dd>
                </div>
            </dl>
        </div>
    </div>
