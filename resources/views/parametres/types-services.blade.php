...
@section('title', 'Types de services - Paramètres - Abedrhman')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow border border-gray-100 dark:border-gray-700 p-6">
                ...
                <form action="{{ route('parametres.types-services.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                    ...
                    <div>
                        <label for="prix" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Prix par défaut (DH)</label>
                        <input type="number" name="prix" id="prix" min="0" step="0.01" placeholder="0.00"
                            class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-colors">
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Ce prix sera utilisé par défaut lors de la création d'un service de ce type</p>
                        @error('prix')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow border border-gray-100 dark:border-gray-700 overflow-hidden">
                <div class="p-4 sm:p-6 border-b border-gray-100 dark:border-gray-700 bg-teal-50 dark:bg-teal-900/20">
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $typesServices->count() }} type(s) configuré(s)</p>
                </div>
                ...
                <div class="divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    <div class="p-4 sm:p-6 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        <form action="{{ route('parametres.types-services.update', $type) }}" method="POST" class="space-y-4">
                            @csrf
                            @method('PUT')
                            <div class="flex flex-col gap-4">
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="code" value="{{ $type->code }}" required 
                                            class="w-full px-3 py-2 text-sm rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-colors">
                                    </div>
                                </div>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Prix par défaut (DH)</label>
                                        <input type="number" name="prix" value="{{ $type->prix }}" min="0" step="0.01"
                                            class="w-full px-3 py-2 text-sm rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-colors">
                                    </div>
                                    <div>
                                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Description</label>
                                        <textarea name="description" rows="2"
                                            class="w-full px-3 py-2 text-sm rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-colors resize-none">{{ $type->description }}</textarea>
                                    </div>
                                </div>
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-2">
                                        <input type="checkbox" name="is_active" id="is_active_{{ $type->id }}" {{ $type->is_active ? 'checked' : '' }}
                                            class="w-4 h-4 text-teal-600 bg-gray-100 dark:bg-gray-700 border-gray-300 dark:border-gray-600 rounded focus:ring-teal-500 focus:ring-2">
                                        <label for="is_active_{{ $type->id }}" class="text-sm text-gray-700 dark:text-gray-300">Actif</label>
                                        @if(!$type->is_active)
                        ...
                        <form action="{{ route('parametres.types-services.destroy', $type) }}" method="POST" class="mt-2 flex justify-end" 
