...
@section('title', 'Tarification DOM - Paramètres - Abedrhman')
...
<div class="space-y-6 sm:space-y-8">
    ...
    <form action="{{ route('parametres.tarifications-dom.update') }}" method="POST">
        @csrf
        @method('PUT')
                            ...
                            <td class="px-4 py-4 whitespace-nowrap bg-gray-50 dark:bg-gray-800 sticky left-0 border-r border-gray-200 dark:border-gray-700">
                                <div class="flex items-center gap-3">
                                    <div class="h-10 w-10 rounded-lg bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center">
                                        <span class="text-sm font-bold text-emerald-600 dark:text-emerald-400">{{ $offre->duree_mois }}m</span>
                                    </div>
                                    <div>
    ...
    <div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-8 text-center">
        ...
        <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <a href="{{ route('parametres.offres-dom') }}" class="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-medium rounded-lg transition-colors flex items-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
